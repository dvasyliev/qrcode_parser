...
const transform2 = (definitions, fields) => {
  ...
  definitions.forEach(definition => {
    ...
    if (!_.has(field, 'value')) {
      model[definition.name] = _.has(definition, 'children') ? transform2(definition.children, []) : null
    } else if (_.has(definition, 'children') && _.isArray(field.value)) {
      model[definition.name] = transform2(definition.children, field.value)
    }
  })
